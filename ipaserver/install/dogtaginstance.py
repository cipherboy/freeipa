...
class DogtagInstance(service.Service):
    ...
    def secure_ajp_connector(self):
        ...
        # Wether or not we should rewrite the tomcat server.xml file with
        # our changes.
        rewrite = False
        ...
        for connector in connectors:
            ...
            if (secretattr not in connector.attrib
                ...
                # during pkispawn. In the former safe, it is always safe to
                # change the AJP secret value. In the latter scenario, we
                # should always ensure the AJP connector is the one we use
                # with httpd, as we don't officially support using multiple
                # AJP adapters for non-IPA uses.
                #
